...
export default {
  ...
  methods: {
     ...
     **/
    videoSwitch () {
      if (this.rtc.isLocalVideoPaused()) {
    ...
    async HcallPersonal (id) {
      ...
      let res = await this.axios({
        method: 'post',
        url: 'https://testsip.teampro.work/youme_h323_gateway/reg_user',
        params: {
          appkey: this.YM_APPKEY,
          curtime,
          checksum
      ...
      let resCall = await this.axios({
        method: 'post',
        url: 'https://testsip.teampro.work/youme_h323_gateway/join_room',
        params: {
          appkey: this.YM_APPKEY,
          curtime,
          checksum
      ...
      this.memberIds.map(item => {
        if (item.userId != this.USERID) {
          this.rtc.requestUserStream(item.userId).then( (stream) => {
            ...
            if (item.userId.indexOf('@') > -1) {
              userid = item.userId.substring(item.userId.indexOf(':') + 1, item.userId.indexOf('@'))
            }
            this.$refs.videoGroup.playVideo(userid, stream)
          })
        }
